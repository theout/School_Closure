...
// Define has hasSchoolAgeChild and hasYoungChild
...
keep if age >= 21 // Remove the children

gen time = (year-2020)*12+month

// Hours worked last week
gen hoursworking = ahrsworkt if ahrsworkt != 999 // Remove NIU obs

// Employment
gen employment = empstat if empstat != 0 // Remove NIU obs

gen inlabforce = (labforce == 2) if labforce != 0 // In labor force status
gen atwork = (employment == 10) // At work status
gen notatwork = (employment == 12) // Not at work status
gen unemployed = (employment == 21 | employment == 22) // unemployment status

// Occupation & Industry
gen occ = occ2010 if occ2010 != 9999 // Remove NIU obs
gen ind = ind1950 if ind1950 != 0 // Remove NIU obs

// Personal characteristics
gen i_female = (sex == 2) if sex != 9 // Generate female dummy
gen i_white = (race == 100) // white race dummy
gen i_black = (race == 200) // black race dummy
gen i_asian = (race == 651) // asian race dummy
gen i_married = (marst == 1) if marst != 9 // married dummy
